...
> **Symlinks:** `plugins/mega-eval/` uses symlinks to avoid duplicating skill files. Preserve them when copying — broken symlinks will cause the plugin to fail path resolution silently.
...
> **Fork maintainers:** the issue links above point to the canonical `Cartooli/mega-eval-skill` repository. Update the `owner/repo` slug in these URLs if you run your own tracker.
...
| Path | Contents |
|------|----------|
| [`examples/sample-run/`](examples/sample-run/) | Complete pipeline output from mega-eval evaluating itself — every file was generated by the pipeline, nothing hand-edited |
| [`examples/run-feedback/`](examples/run-feedback/) | Fictional `run-log.md` and learning-promotion samples showing the run-feedback format |

## Contributing

See [CONTRIBUTING.md](CONTRIBUTING.md) for how to file issues, submit pull requests, and understand the repo layout. Security issues should follow [SECURITY.md](SECURITY.md).
